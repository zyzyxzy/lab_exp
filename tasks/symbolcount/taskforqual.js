var info_ = {
  subject: jsPsych.randomization.randomID(15), // random subject id
...
// TASK PARAMETERS
const trials = 20; // the total number of trials to present
...
var itis = [500, 600, 700, 800, 900, 1000]; // intertrial intervals
var choices = [
  "1",
  "2",
  "3",
...
function determine_sequence(reps, symbols, trial_difficulty, verbose) {
  ...
  for (i = 0; i < reps; i++) {
    if (i == 0) {
      // randomly determine first symbol
      n_symbol = random_choice(n_symbols);
      sequence.push(n_symbol);
    } else {
      // determine subsequent symbols
      if (switch_trial_idx.includes(i)) {
        // if switch, change symbol
...
var instructions = {
  type: "instructions",
  pages: [
    "Welcome!<p>Click next or press the right arrow key to proceed.</p>",
    "<p>In this task, you'll see sequences of " +
      "dollar signs ($) and hash/pound symbols (#). <p>Your goal is to keep a count of " +
...
var fixation = {
  ...
  stimulus: "https://zyzyxzy.github.io/lab_exp/tasks/symbolcount/fixation_white.png", // dot as fixation
  stimulus_height: 30,
  stimulus_width: 30,
...
var symbols_sequence = {
  // determine sequence of symbols within a trial
  timeline: [
    {
      ...
      }, // function is needed to dynamically change value on each trial
      ...
      on_finish: function (data) {
        ...
        n_dollar = jsPsych.data
          .get()
          .filter({ stimulus: symbols[0]["symbol"], n_trial: n_trial })
          .count();
        data.n_dollar = n_dollar;
        n_hash = jsPsych.data
          .get()
          .filter({ stimulus: symbols[1]["symbol"], n_trial: n_trial })
          .count();
        data.n_hash = n_hash;
        data.rep = reps;
  ...
  sample: {
    // custom sampling function to generate pseudorandom sequence of symbols on each trial
    type: "custom",
    fn: function (samples) {
...
var response = {
  ...
  timeline_variables: [
    {
      symbol:
        '<div style="transform: translateY(-30px); font-size:25px;"> How many $ symbols </div>',
    },
    {
      symbol:
        '<div style="transform: translateY(-30px); font-size:25px;"> How many # symbols </div>',
  ...
  on_finish: function (data) {
    ...
    responses.push(
      parseInt(
        choices[parseInt(jsPsych.data.get().last(1).values()[0].button_pressed)]
      )
    );
    if (debug) {
...
var feedback = {
  ...
  stimulus: function () {
    text =
      "<p>Actual counts</p><p>" + n_dollar + " $ and " + n_hash + " #<p></p>";
    if (show_performance) {
      counts =
        "<p>Your counts</p><p>" +
        responses[0] +
...
var trial = {
  ...
  conditional_function: function () {
    // skip this object/timeline if time has elapsed
    var max_tasktime_ms = max_tasktime_minutes * 60 * 1000;
    var start_time = jsPsych.data.get().first().select("time_elapsed")
      .values[0];
    var current_time = jsPsych.data.get().last().select("time_elapsed")
      .values[0];
...
var debrief_block = {
  ...
  stimulus: function () {
    var html =
      "<p>Click the Finish button below to end the experiment. Thank you! </p>";
